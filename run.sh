...
/**
 * Environment variables set up in BASH ${VAL-DEFAULTVALUE}
 */
RTMP_CONNECTIONS=${RTMP_CONNECTIONS-3}
RTMP_STREAM_NAMES=${RTMP_STREAM_NAMES-live}
...
cat >>${NGINX_CONFIG_FILE} <<!EOF
        application ${STREAM_NAME} {
            live on;
            record off;
            on_publish http://localhost:${HTTP_PORT}/on_publish;
!EOF
if [ "${HLS}" = "true" ]; then
cat >>${NGINX_CONFIG_FILE} <<!EOF
            hls on;
